...
if(isset($_REQUEST['eEmail'])){
	$eEmail = mysqli_real_escape_string($conn, trim($_REQUEST['eEmail']));
    $ePassword = mysqli_real_escape_string($conn, trim($_REQUEST['ePassword']));
    $sql ="SELECT e_email, e_password FROM expectantlogin_tb
    WHERE e_email = '".$eEmail."' AND e_password = 
     '".$ePassword."' limit 1";
     $result = $conn->query($sql);
if($result->num_rows == 1){
	$_SESSION['is_login'] = true;
	$_SESSION['eEmail'] = $eEmail;
	echo "<script> location.href='expectantprofile.php';</script>";
	exit;
} else{
...
<p class="text-center" style="font-size:20px;"><i class="fa fa-user-plus" aria-hidden="true"></i>EXPECTANT LOGIN</p>
<!-- FORM CREATION-->
...
<label for="email" class="font-weight-bold pl-2">EMAIL</label><input type="email" class="form-control" placeholder="email" name="eEmail">
</div>
<div class="form-group">
<i class="fa fa-unlock" aria-hidden="true"></i>
<label for="pass" class="font-weight-bold pl-2">PASSWORD </label><input type="password" class="form-control" placeholder="password" name="ePassword">
</div>
